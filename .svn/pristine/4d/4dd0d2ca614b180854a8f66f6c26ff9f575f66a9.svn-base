...
//                    Consulta para obtener lista de tickets PROCESANDO y sede ordenados por fecha de recepccion 
                        ...
                        consultaTickets.setParameter("sede", Integer.parseInt(e.getUsuarioVO().getSede_id()));
                    ...
                    <table class="table table-hover" id="tabla" >
                        ...
                        <tbody>
                            <%
                                for (Ticket t : l) {
                                    if (t.getTickEstado().equals("PROCESANDO") && t.getSedeId().getSedeId() == Integer.parseInt(e.getUsuarioVO().getSede_id())) {
                                ...
                                for (TicketClienteservicio tcl : t.getTicketClienteservicioList()) {
                                    for (ServiciosExamenes se : tcl.getClseId().getServId().getServiciosExamenesList()) {
                                        if (se.getExamId().getExamTipo().equals("LABORATORIO")) {
                                            ...
                                            String color = !procesado ? "color: #006dcc" : "color: #00CE6F";
                                            String icon = !procesado ? "circle-arrow-right" : "ok-circle";
                                            examenes += "<span class='glyphicon glyphicon-" + icon + "' style='" + color + "'></span>" + se.getExamId().getExamDescripcion() + " <br />";
...
//        col_2: 'select',
        ...
        btn_reset: true,
        paging: true,
        results_per_page: ['Resultados por Pag.', [100]],
        paging_length: 100,
        rows_counter: true,
        loader: true,
        status_bar: true,
